...
const getMatchList = async (
	...
	begIndex: string,
	endIndex: string
): Promise<LcuMatchList | null> => {
	try {
		const res: LcuMatchList = await invoke("get_match_list", {
			puuid: puuid,
			begIndex: begIndex,
			endIndex: endIndex,
		});
...
export const formatMathchInfo = async (
	puuid: string,
	begIndex: string,
	endIndex: string
) => {
	...
	if (
		matchList === null ||
		matchList?.games?.games?.length === 0 ||
		matchList?.games?.games === undefined
	) {
		return [];
	}
