...
export const querySummonerInfo =async (sumId?:number) => {
    const summonerInfo:lcuSummonerInfo =sumId?await invoke ('get_other_sum',{summonerId:String(sumId)}): await invoke('get_cur_sum');
    ...
    const [rankPoint,excelChamp] =
    await Promise.all([
      getRankPoint(puuid),
      getExcelChamp(`${summonerId}`)])

    return {
        summonerInfo,rankPoint,excelChamp
    }
}
...
const getRankPoint = async (puuid: string) => {
    ...
    let RANKED_SOLO = rankSolo.tier === "" ? '未定级' : `${trans2Chinese(rankSolo.tier)}${dealDivsion(rankSolo.division)} ${rankSolo.leaguePoints}`
    let RANKED_FLEX_SR = rankSr.tier === "" ? '未定级' : `${trans2Chinese(rankSr.tier)}${dealDivsion(rankSr.division)} ${rankSr.leaguePoints}`
    let RANKED_TFT = rankTft.tier === "" ? '未定级' : `${trans2Chinese(rankTft.tier)}${dealDivsion(rankTft.division)} ${rankTft.leaguePoints}`
